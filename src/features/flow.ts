...
// add (§4) and the «повторение» session (§5/§6) — and reused by the scheduler.
 ...
 * middleware (the review session) and inside a conversation (add) — it only needs
 * `ctx.api` + `ctx.chat`.
 */
...
/**
 * Reset the session to IDLE, clearing any in-progress flow state.
